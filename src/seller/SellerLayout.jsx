...
const SELLER_NAV = [
  { label: 'Dashboard', path: '/seller', icon: '📊', exact: true },
  { label: 'My Products', path: '/seller/products', icon: '📦', match: 'products-list' },
  { label: 'Add Product', path: '/seller/products/new', icon: '➕', exact: true },
  { label: 'Enquiries', path: '/seller/enquiries', icon: '✉️' },
];
...
const SellerLayout = () => {
  ...
  if (user.role !== 'SELLER') {
    return <Navigate to={user.role ? '/admin' : '/seller/login'} replace />;
  }

  const isActive = (item) => {
    const { pathname } = location;
    if (item.exact) return pathname === item.path;
    if (item.match === 'products-list') {
      if (pathname === '/seller/products/new') return false;
      return pathname === '/seller/products' || /^\/seller\/products\/[^/]+\/edit$/.test(pathname);
    }
    return pathname === item.path || pathname.startsWith(`${item.path}/`);
  };

  const currentPage =
    [...SELLER_NAV].reverse().find((item) => isActive(item))?.label || 'Seller';

  const SidebarContent = ({ onNavigate }) => (
    <div className="flex flex-col h-full min-h-0">
      <div className="p-5 border-b border-white/10">
        <Link to="/seller" onClick={onNavigate} className="block">
          <p className="font-bold text-lg text-white leading-tight">Seller Dashboard</p>
          <p className="text-xs text-white/50 mt-1 truncate">{user.businessName || user.name}</p>
        </Link>
      </div>
          ...
          <div className="flex items-center justify-between px-4 sm:px-6 py-3 gap-3">
            <div className="flex items-center gap-2 sm:gap-3 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <p className="text-xs text-slate-500 hidden sm:block">Manage your marketplace listings</p>
              </div>
            </div>
            <div className="flex items-center gap-2 flex-shrink-0">
              <Link
                to="/marketplace"
                className="hidden sm:inline-flex text-sm text-teal-700 font-medium px-3 py-1.5 rounded-lg hover:bg-teal-50"
              >
                Marketplace
              </Link>
              <Link to="/" className="text-sm text-slate-600 hover:text-teal-700 px-2 sm:px-3 py-1.5 rounded-lg hover:bg-stone-50">
                Site
              </Link>
            </div>
          </div>
